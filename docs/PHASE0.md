...
| 10 | Deals (#32 design) | FIXED | Finder on Actions ranked by expected money (scores non-increasing): with Firecrawl run 36220828966, 24 cards; keyless (Firecrawl disconnected) run 36221145903, 28 cards, light green. Real-model pitch (968 chars) → Gmail compose link → marked sent by hand 6 days back → follow-up due on Home. Brand offer read (terms "from their email", red flags), memo fee $650, rate card 4 packages. Nothing to a brand: 0 dashboard emails to anyone but the owner | #36 (someone else's clip could reach the kit / a pitch) |
| 11 | Media kit (#32 design) | PASS | Edit (name, line, handle, about, contact via kit check), publish → version 2 "live now", public `/kit/sheila` logged out with QR + "Work with me" mailto, no private rates in the public JSON, print page "Save as PDF" (1 page rendered headless); `11-kit-*.jpg` | #36 |
...
| 15 | Clean-up | PASS | 12 scheduled TEST posts taken off 04:26 UTC (12/12 gone from Buffer). Clips deleted: 28 + 7 + 5 + 28 (Looks dump), test brand docs removed, 3 voice overs deleted, 2 test deals closed (lost / declined). Posts the owner deleted on TikTok/Instagram: Buffer still says `sent`, the dashboard never re-reads a posted post → lights green, no email, no re-post (checked with Check everything now). The YouTube TEST short stays up: neither the dashboard nor Buffer can delete a published video. Connections kept | — |

| 16 | Looks (#27) | PASS | Dump dmp_hnhy6oar2004 (246 s third-party test video, never scheduled): 28 clips in all 12 Looks incl. 6 grids; Change look (clean → cinematic) re-rendered on the runner, new file, plays. Watermark note now names the creator: "we saw a TikTok watermark for @texasgardenfairyx" (cut.source_check handles=1). The 66 s video could not be re-dumped: the identical file is refused by design | — |
...
13. Someone else's video (held off the calendar) could still be offered for the media kit, put
    in the public kit and linked in a pitch (#36).
